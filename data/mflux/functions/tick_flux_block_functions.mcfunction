...
execute at @e[type=area_effect_cloud,tag=Conduit,sort=random,distance=1..8,limit=1] if score @s flux > @e[type=area_effect_cloud,tag=Conduit,distance=..0.5,limit=1] flux run function mflux:transfer_flux
...
execute if score @s age matches 2 if block ~ ~-1 ~ chiseled_polished_blackstone if entity @a[distance=..1.1] run function mflux:expel_mana
execute if score @s flux matches 6.. at @s if block ~ ~-1 ~ chiseled_polished_blackstone run function mflux:expel_mana
...
### Furnace
execute as @e[tag=Conduit,scores={flux=1..}] if block ~ ~-1 ~ furnace{Items:[{Slot:0b}],BurnTime:0s} run function mflux:fuel_furnace

### Hopper Funnels
execute if score @s age matches 2 if block ~ ~-1 ~ hopper if entity @e[type=item,distance=0.4..3,nbt={OnGround:1b}] run function mflux:funnel_items

### Clairvoyance / Seeing Glass
execute if score @s age matches 2 if block ~ ~-1 ~ glass run function mflux:clairvoyance

### Levitation
execute if score @s age matches 2 if block ~ ~-1 ~ gold_block positioned ~-0.75 ~ ~-0.75 if entity @a[gamemode=!spectator,dx=0.5,dz=0.5,dy=10] run function mflux:levitate_players
...
execute if score @s age matches 2 if block ~ ~-1 ~ emerald_block if entity @e[type=#majik:hostile,distance=..20] run function mflux:monster_repulsion

### Projectile Repulsion Field - run every frame for more accurate and rapid reflections
execute if block ~ ~-1 ~ diamond_block if entity @e[type=#majik:projectile,distance=..7,nbt=!{inGround:1b},tag=!RepulsionFieldCooldown] run function mflux:projectile_repulsion