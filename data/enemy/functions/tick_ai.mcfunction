...
execute at @e[type=squid,scores={age=1}] run function enemy:make_endersquid
...
## Endersquid Teleport
execute as @e[tag=Endersquid,nbt={HurtTime:10s}] at @s run function spells:point_ender
execute as @e[tag=Endersquid,nbt={HurtTime:9s}] at @s run function spells:point_ender
