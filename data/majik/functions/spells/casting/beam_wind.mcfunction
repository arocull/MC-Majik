...
execute as @e[type=!player,distance=..4,tag=!VectorCalculator,tag=!SpellScroll] at @s run function majik:spells/casting/beam_wind_knockback
...
execute positioned ^ ^ ^0.5 unless block ~ ~ ~ #majik:nonsolid run scoreboard players set @s beam_length 31
execute positioned ^ ^ ^0.5 if block ~ ~ ~ #majik:nonsolid if score @s beam_length matches ..30 run function majik:spells/casting/beam_wind
