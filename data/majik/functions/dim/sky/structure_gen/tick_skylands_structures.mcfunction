...
execute in majik:skylands as @e[type=#majik:docile,nbt={Age:0},predicate=majik:in_skylands,predicate=majik:in_flower_forest] at @s unless entity @e[type=area_effect_cloud,tag=MajikStructureGen,distance=..1000] if predicate majik:random_one_tenth run function majik:dim/sky/structure_gen/summon_well
